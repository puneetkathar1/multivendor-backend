        ...
        .then(async (p) => {
          ...
          if (p.id) {
            try {
              ...
              let addrdetails = await findAddressList(addrId, {
                transaction: t,
              })
              //end
              let fullName = !deliveryId
                ? deliveryAddress.name
                : req.user.firstName
              await mailer.sendInvoiceForCustomer(
                req.body,
                list,
                addrdetails,
                fullName,
    ...
    try {
      db.OrderNotification.findAll({
        ...
        .then((list) => {
          res
            .status(200)
            .json({ success: true, data: list, count: list.length })
        })
        .catch(function (err) {
          next(err)
        })
      ...
      db.Order.findOne({
        ...
        .then((success) => {
          res
            ...
            .json({
              success: true,
              message: 'Successfully Deleted Product from list',
            })
        })
    } catch (err) {
      res.status(500).json({ errors: '' + err })
      ...
      db.Order.findOne({
        ...
        .then((success) => {
          res
            ...
            .json({
              success: true,
              message: 'Successfully Deleted Order from list',
            })
        })
    } catch (err) {
      res.status(500).json({ errors: '' + err })
      ...
      db.Order_Details_Status.findOne({
        ...
        .then((success) => {
          res
            ...
            .json({
              success: true,
              message: 'Successfully add status in the list',
            })
        })
    } catch (err) {
      res
    ...
    try {
      db.Cart_Detail.findOne({
        ...
        .then((success) => {
          res
            .status(200)
            .json({ success: true, message: 'Successfully canceled orderlist' })
        })
    } catch (err) {
      res
        .status(500)
